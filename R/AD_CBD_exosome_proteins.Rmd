...
sets <- lapply(sets, function(x) rename(x, c("Gene names" = "Genes", "p value" = "p-value")))
sets[[1]] <- rename(sets[[1]], c("AD_Control fold diff" = "AD/Control Fold Change"))
sets[[2]] <- rename(sets[[2]], c("CBD_Control_Fold difference" = "CBD/Control Fold Change"))
sets[[3]] <- rename(sets[[3]], c("AD_CDB_FoldDifference" = "AD/CBD Fold Change"))
sets <- lapply(sets, function(x) x[x$`p-value` < 0.05, ])
...
```{r}
combined_set <- cbind.na(sets[[1]][ , c(1, 2)], sets[[2]][ , c(1, 2)], sets[[3]][ , c(1, 2)])
