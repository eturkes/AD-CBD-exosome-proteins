...
sets <- lapply(sets, function(x) replace(x, is.na(x), 1)) # Replace remaining NA with 1.
...
merged_set <- replace(merged_set, is.na(merged_set), 1) # Replace remaining NA with 1.
...
merged_top <- replace(merged_top, is.na(merged_top), 1) # Replace remaining NA with 1.
