...
sets <- lapply(sets, function(x) replace(x, is.na(x), 0)) # Replace remaining NA with 0.
...
merged_set <- replace(merged_set, is.na(merged_set), 0) # Replace remaining NA with 0.
...
merged_top <- replace(merged_top, is.na(merged_top), 0) # Replace remaining NA with 0.
