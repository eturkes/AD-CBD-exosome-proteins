...
merged_set <- replace(merged_set, is.na(merged_set), as.double("NA")) # Properly type remaining NA.
...
Due the large number of `NA` from not passing the p-value cutoff, clustering is not used in heatmaps.
...
merged_top <- replace(merged_top, is.na(merged_top), as.double("NA")) # Properly type remaining NA.
...
rownames(merged_top) <- top_genes
pheatmap(merged_top, cluster_rows = FALSE, cluster_cols = FALSE, angle_col = 315)
```
...
rownames(merged_top) <- top_genes
pheatmap(
  merged_top, cluster_rows = FALSE, cluster_cols = FALSE,
  angle_col = 315, color = colorRampPalette(rev(brewer.pal(n = 7, name = "Reds")))(100))
```
...
pheatmap(
  merged_top, cluster_rows = FALSE, cluster_cols = FALSE,
  angle_col = 315, color = colorRampPalette(rev(brewer.pal(n = 7, name = "Blues")))(100))
```
