...
top_abs <- apply(merged_top, 1, function(x) abs(max(x)))
top_genes <- top_genes[order(-top_abs)]
...
set.seed(1)
upset(merged_top, empty.intersections = "on", query.legend = "bottom", queries = queries)

merged_top <- t(merged_top)
colnames(merged_top) <- as.character(unlist(merged_top[1, ]))
merged_top <- merged_top[-1, ]
mode(merged_top) = "numeric"
merged_top <- as.data.frame(merged_top)
