...
```{r}
AD_control <- head(merged_set[order(-abs(merged_set$log2_AD_Control_Fold_Change))], n = 20)
CBD_control <- head(merged_set[order(-abs(merged_set$log2_CBD_Control_Fold_Change))], n = 20)
AD_CBD <- head(merged_set[order(-abs(merged_set$log2_AD_CBD_Fold_Change))], n = 20)
...
pheatmap(
  merged_top, angle_col = 0, fontsize = 9, color = color, cluster_cols = FALSE, breaks = breaks)
```

UpSet plots [@conway_upsetr_2017] help visualize set interactions among entities, similar to a Venn diagram.
...
Note that this provides no indication of directionality (i.e. up vs. downregulated), however the bottom right scatter plot is able to summarize that.
...
upset(
  ...
  attribute.plots = list(gridrows = 45, plots = list(
    ...
    list(
      ...
      y = "log2_CBD_Control_Fold_Change", queries = TRUE)), ncols = 2))
```
...
upset(
  merged_top, nsets = dim(merged_top)[2], mb.ratio = c(0, 0.1),
...
```{r}
AD_control <- head(merged_set[order(-abs(merged_set$log2_AD_Control_Fold_Change))], n = 30)
CBD_control <- head(merged_set[order(-abs(merged_set$log2_CBD_Control_Fold_Change))], n = 30)

merged_top <- Reduce(function(...) merge(..., all = TRUE), list(
  AD_control[ , c(1, 2)], CBD_control[ , c(1, 3)]))
merged_top <- merged_top[!duplicated(merged_top$Genes), ]
merged_top <- merged_top[!is.na(merged_top$Genes), ]
merged_top <- merged_top[!merged_top$Genes == "", ]
...
```{r}
AD_control <- head(merged_set[order(-merged_set$log2_AD_Control_Fold_Change)], n = 20)
CBD_control <- head(merged_set[order(-merged_set$log2_CBD_Control_Fold_Change)], n = 20)
AD_CBD <- head(merged_set[order(-merged_set$log2_AD_CBD_Fold_Change)], n = 20)
...
color[1] <- "#DDDDDD"
pheatmap(merged_top, angle_col = 0, fontsize = 9, color = color, cluster_cols = FALSE)
```

```{r}
AD_control <- head(merged_set[order(-merged_set$log2_AD_Control_Fold_Change)], n = 30)
CBD_control <- head(merged_set[order(-merged_set$log2_CBD_Control_Fold_Change)], n = 30)
...
datatable_download(merged_spec) %>%
  formatRound(columns = c(2:dim(merged_spec)[2]), digits = 2)

# Generate heatmap, using grey for 0 in color palette to indicate NA values.
merged_spec <- as.matrix(head(merged_spec, n = 50))
rownames(merged_spec) <- head(merged_genes, n = 50)
merged_spec <- replace(merged_spec, is.na(merged_spec), 0)
color <- colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
color[50] <- "#DDDDDD"
breaks <- c(
  seq(min(merged_spec), -0.001, by = abs(min(merged_spec)) / 50), 0,
  seq(0.001, max(merged_spec), by = abs(max(merged_spec)) / 50))
pheatmap(
  merged_spec, angle_col = 0, fontsize = 9, color = color, cluster_cols = FALSE, breaks = breaks)
```
