---
title: "AD Fractions No Scaling Factor"
author:
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(
    "..", "results", "AD-fractions-no-scaling-factor.html"
  ))})
---
...
MISEV_groups <- read.delim(file.path(assets_dir, data_name, "MISEV-groups.tsv"), header = FALSE)
...
no_NA_data <- sub_data
no_NA_data[is.na(no_NA_data)] <-
  min(assay(data), na.rm = TRUE) - (min(assay(data), na.rm = TRUE) * 0.1)
...
```{r, fig.height = 30, fig.width = 2}
combined_data <- as.matrix(rowMeans(sub_data))
...
for (i in seq(ncol(MISEV_groups))) {
  ...
  rownames(new_row) <- paste0(trimws(MISEV_groups[1, i]), "*")
