...
### EV Proteins vs. Contaminants

```{r}
MISEV <- read.delim(file.path(assets_dir, data_name, "MISEV-tidy.tsv"))

proteins <- as.vector(unlist(MISEV))
proteins <- proteins[proteins != ""]

sub_data <- assay(data[rowData(data)$Gene %in% proteins, ])
datatable_download(sub_data)

remove <- unlist(MISEV) %notin% rownames(sub_data)
proteins <- unlist(MISEV)
