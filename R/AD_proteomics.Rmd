...
packages <- c("proteus", "Amelia", "DT", "data.table", "tidyr", "limma", "RColorBrewer", "ggplot2")
invisible(suppressPackageStartupMessages(lapply(packages, library, character.only = TRUE)))
source(file.path(getwd(), "utils.R"))

color <- c(brewer.pal(4, "Blues"), brewer.pal(4, "Reds"))
...
data$metadata$condition <- c(rep("Contr", 4), rep("AD", 5))
levels(data$metadata$condition) <- c("Contr", "AD")
data$conditions <- c("Contr", "AD")
...
```{r, all-de}
mat <- log10(data$tab)
conditions <- data$metadata$condition
design <- model.matrix(~0 + conditions, data = as.data.frame(mat))
fit <- lmFit(mat, design = design)
cont_mat <- makeContrasts(conditionsAD - conditionsContr, levels = design)
cont_fit <- eBayes(contrasts.fit(fit, cont_mat), trend = TRUE, robust = TRUE)
table <- topTable(cont_fit, coef = 1, number = Inf)
datatable_custom(table)
```
...
data$metadata$condition <- c(rep("Contr", 4), rep("AD", 4))
levels(data$metadata$condition) <- c("Contr", "AD")
data$conditions <- c("Contr", "AD")
...
```{r, vis}
# Some of these reshaping commands are not compatible with data frames, so use data table at first.
df <- data.table(rownames(data$tab), log10(data$tab))
df <- melt.data.table(df, id = "V1", variable.name = "sample")
df <- spread(df, "V1", "value")
setDF(df)

df <- data.frame(sample = df[ , 1], condition = data$metadata$condition, df[ , 2:ncol(df)])
df$condition <- factor(df$condition, levels = c("Contr", "AD"))
df_sub <- df[ , c(1:2, which(colnames(df) == "P10636"))]
colnames(df_sub)[3] <- "MAPT"

ggplot(df_sub, aes(condition, y = MAPT)) + 
  geom_violin() +
  geom_jitter(height = 0, width = 0, aes(colour = sample)) +
