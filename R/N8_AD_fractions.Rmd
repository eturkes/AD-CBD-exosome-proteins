...
```{r}
# Step 1 in flowchart.
# Replace proteins missing entirely in a condition with minimum value per sample in random samples.
# -------------------------------------------------------------------------------------------------
...
if (file.exists(rds)) {
  ...
  for (fraction in unique(data$condition)) {
    ...
    set.seed(grep("[:digit:]", fraction))
    col <- sample(which(data$condition == fraction), length(replace), TRUE)
    for (i in seq_along(replace)) {
      assay(data)[replace[i], col[i]] <- min(assay(data)[ , col[i]], na.rm = TRUE)
    }
  }
  saveRDS(data, rds)
}
...
![](../tmp/cache/oxford-ms-data-april-2022/fractions/NormalyzerDE/pngs/10_boxplot.png)
![](../tmp/cache/oxford-ms-data-april-2022/fractions/NormalyzerDE/pngs/11_rel_log_expr.png)
