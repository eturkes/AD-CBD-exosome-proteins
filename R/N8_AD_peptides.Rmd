...
packages <- c(
  ...
  "ComplexHeatmap", "tibble", "readxl", "khroma", "NormalyzerDE"
)
...
```{r}
data <- read.delim(file.path(assets_dir, "oxford-ms-data-april-2022", "20220404_DOB_AllPatients_v1.pr_matrix.tsv"))
tmp1 <- data[ , 1:10]
...
colnames(data) <- c(colnames(data)[1:10], colnames(data2)[5:ncol(data2)])
rm(data2)
...
data <- data[
  , c(
      1:10,
      (order(substring(colnames(data)[LFQ_columns], nchar(colnames(data))[LFQ_columns])) + 10),
      72:ncol(data)
    )
]
