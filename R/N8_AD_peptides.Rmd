...
The data here will be referenced using the name `peptides`.
...
packages <- c(
  ...
  "ComplexHeatmap", "tibble", "khroma", "NormalyzerDE"
)
...
cache_dir <- file.path("..", "tmp", "cache", "peptides")
...
```{r}
data <- read.delim(
  file.path(assets_dir, "oxford-ms-data-april-2022", "20220404_DOB_AllPatients_v1.pr_matrix.tsv")
)
tmp1 <- data[ , 1:10]
tmp2 <- as.data.frame(data[ , 11])
colnames(tmp2) <- colnames(data)[11]
tmp3 <- data[ , 12:19]
tmp4 <- data[ , 20:ncol(data)]
data <- cbind(tmp1, tmp3, tmp2, tmp4)
rm(tmp1, tmp2, tmp3, tmp4)

data <- data[ , c(1:10, c(1:32, 41:48, 33:40, 57:64, 49:56) + 10)]
colnames(data) <- c(
  colnames(data)[1:10],
...
data$name <- paste(data$gene_names, data$Precursor.Id, sep = "_")
data$ID <- data$Protein.Ids

LFQ_columns <- 11:71
data <- data[ , c(1:10, (order(substring(colnames(data)[LFQ_columns], 2, 2)) + 10), 72:ncol(data))]

experimental_design <- data.frame(
  label = colnames(data)[11:71],
...
complete_cases <- filter_proteins(data, "complete") # Keep an object with only complete values.
```
...
![](../tmp/cache/peptides/NormalyzerDE/pngs/10_boxplot.png)
![](../tmp/cache/peptides/NormalyzerDE/pngs/11_rel_log_expr.png)
...
rm(
  VSN_Normalised, VSN_Normalised_Non_Missing_Data,
  Log_Transform_Only, Log_Transform_Only_Non_Missing_Data
)
# --------------------------------------------------
```

### Normalised Data Without Imputation
...
```{r}
data_sub <- assay(data[rowData(data)$gene_names == "MAPT", ])
colnames(data_sub) <- data$label

datatable_download(data_sub)

rownames(data_sub) <- rowData(data[rowData(data)$gene_names == "MAPT", ])$Stripped.Sequence
...
rm(Imputed_Data, Unimputed_Data, Non_Missing_data)

hist(assay(data), n = 100)
```

### Normalised Data After Imputation
