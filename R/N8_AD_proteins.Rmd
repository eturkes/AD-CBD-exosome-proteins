...
packages <- c(
  ...
  "biomaRt", "NormalyzerDE", "stringi", "openxlsx"
)
invisible(suppressPackageStartupMessages(lapply(packages, library, character.only = TRUE)))
source(file.path("utils.R"))
conflict_prefer("rowMedians", "Biobase", quiet = TRUE)
conflict_prefer("colMedians", "matrixStats", quiet = TRUE)
...
results_dir <- file.path("..", "results", "N8_AD_proteins")
...
for (fraction in unique(data$condition)) {
  ...
  print(
    ggplot(rowmeans, aes(x)) +
      geom_density() +
      theme_bw() +
      labs(x = "log2 Protein Intensity", title = sub("\\.", " ", fraction)) +
...
```{r, fig.height = 10}
select_prot <- read.delim(file.path(assets_dir, "gene-sets", "select-proteins.tsv"))

heatmap_mat <- assay(data)[rownames(data) %in% select_prot$F1.3, ]
colnames(heatmap_mat) <- data$nice_names
heatmap_mat <- t(apply(heatmap_mat, 1, function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1)))

datatable_download(heatmap_mat)
...
heatmap_mat <- assay(data)[rownames(data) %in% select_prot$F4.6, ]
colnames(heatmap_mat) <- data$nice_names
heatmap_mat <- t(apply(heatmap_mat, 1, function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1)))
...
heatmap_mat <- assay(data)[rownames(data) %in% select_prot$F7.8, ]
colnames(heatmap_mat) <- data$nice_names
heatmap_mat <- t(apply(heatmap_mat, 1, function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1)))
...
```{r}
additional <- c(
  "endosomal vesicle fusion", "autophagosome-lysosome fusion",
  "ubiquitin-specific protease binding", "exocyst",
  "EARP complex"
)
...
detection_list <- vector("list", length(gene_sets))
for (i in seq_along(detection_list)) {
  detection_list[[i]] <- assay(data[rownames(data) %in% unlist(geneIds(gene_sets)[i])])

  detection_list1 <- detection_list[[i]][ , 1:56]
  detection_list1 <- t(apply(detection_list1, 1, tapply, gl(7, 8), median))
  detection_list2 <- detection_list[[i]][ , 57:ncol(detection_list[[i]])]
  detection_list2 <- apply(detection_list2, 1, tapply, gl(1, 5), median)
  detection_list[[i]] <- cbind(detection_list1, detection_list2)
  colnames(detection_list[[i]]) <- paste0("Fraction ", 1:8)

  detection_list[[i]] <- detection_list[[i]][
    order(rowMedians(detection_list[[i]]), decreasing = TRUE),
  ]

  undetected <- unique(
    unlist(geneIds(gene_sets)[i])[unlist(geneIds(gene_sets)[i]) %notin% rownames(data)]
  )
  undetected <- undetected[undetected != ""]
  undetected <- undetected[order(undetected)]
  if (length(undetected) > 0) {
    tmp <- matrix(
      nrow = length(undetected), ncol = ncol(detection_list[[i]]),
      dimnames = list(undetected, colnames(detection_list[[i]]))
    )
    detection_list[[i]] <- rbind(detection_list[[i]], tmp)
  }

  file_name <- trimws(sub("/", "-", names(gene_sets)[i]))
  write.csv(detection_list[[i]], file.path(results_dir, "johnson", paste0(file_name, ".csv")))
}
names(detection_list) <- trimws(sub("/", "-", names(gene_sets)))
write.xlsx(
  detection_list, file.path(results_dir, "johnson", "johnson.xlsx"), rowNames = TRUE, keepNA = TRUE
)
...
colnames(heatmap_data) <- paste0("Fraction ", 1:8)

order <- order(rowMedians(heatmap_data), decreasing = TRUE)

datatable_download(heatmap_data)
...
## Abundant Cell-type Markers

```{r}
heatmap_color2 <- colorRamp2(
  c(min(assay(data)), (max(assay(data)) + min(assay(data))) / 2, max(assay(data))),
  c("#0077BB", "#FFFFFF", "#CC3311")
)

ordered_fractions <- vector("list", length(unique(experimental_design$condition)))
proteins <- c()
for (i in seq_along(ordered_fractions)) {
  data_sub <- data[ , which(experimental_design$condition == paste0("Fraction ", i))]
  ordered_fractions[[i]] <- assay(data_sub[order(rowMedians(assay(data_sub)), decreasing = TRUE), ])

  heatmap_data <- ordered_fractions[[i]][1:10, ]
  proteins <- c(proteins, rownames(heatmap_data))
  set.seed(1)
  draw(
    Heatmap(
      heatmap_data,
...
```{r, fig.width = 15}
markers <- read.delim(file.path(assets_dir, "gene-sets", "markers.tsv"))
...
### Each Cell-type Sorted by Abundance

```{r, fig.width = 15}
protein_order <- assay(data[rownames(data) %in% markers$gene, ])
protein_order <- rownames(protein_order)[order(rowMedians(protein_order), decreasing = TRUE)]
markers <- markers[match(protein_order, markers$gene), ]
...
### Most Abundant Markers

```{r, fig.width = 15}
proteins <- markers$gene[1:30]
proteins_cell_type <- paste(markers$cluster[1:30], markers$gene[1:30], sep = "_")
proteins_cell_type <- proteins_cell_type[order(proteins_cell_type)]
proteins <- proteins[order(proteins_cell_type)]
...
table(markers$cluster[1:30])
table(markers$cluster)
table(markers_orig$cluster)
```
...
bar_colour <- colour[c(2, 5)]
names(bar_colour) <- c("Undetected", "Detected")

datatable_download(df)

detection_list <- vector("list", length(unique(markers_orig$cluster)))
for (i in seq_along(detection_list)) {
  markers_sub <- markers_orig[markers_orig$cluster == unique(markers_orig$cluster)[i], ]
  detection_list[[i]] <- assay(data[rownames(data) %in% markers_sub$gene, ])
  ...
  markers_match <- markers_sub[match(rownames(detection_list[[i]]), markers_sub$gene), ]
  detection_list[[i]] <- cbind(detection_list[[i]], markers_match[ , 2], markers_match[ , 5])
  colnames(detection_list[[i]]) <- c(
    colnames(detection_list[[i]])[1:8], "Marker Gene Avg. log2FC", "Marker Gene Adj. P-value"
  )

  undetected <- markers_sub[markers_sub$gene %notin% rownames(data), ]
  undetected <- undetected[order(undetected$avg_log2FC, decreasing = TRUE), ]
  if (length(undetected) > 0) {
    tmp <- matrix(
      nrow = nrow(undetected), ncol = 8,
      dimnames = list(rownames(undetected), colnames(detection_list[[i]])[1:8])
    )
    tmp <- cbind(tmp, undetected[ , c(2, 5)])
    colnames(tmp) <- c(colnames(tmp)[1:8], "Marker Gene Avg. log2FC", "Marker Gene Adj. P-value")
    detection_list[[i]] <- rbind(detection_list[[i]], tmp)
  }

  write.csv(
    detection_list[[i]],
    file.path(results_dir, "markers", paste0(unique(markers_sub$cluster), ".csv"))
  )
}
names(detection_list) <- unique(markers_orig$cluster)
write.xlsx(
  detection_list, file.path(results_dir, "markers", "markers.xlsx"), rowNames = TRUE, keepNA = TRUE
)
...
### Each Cell-type Sorted by Abundance (log2FC > 1)
...
### Most Abundant Markers (log2FC > 1)
...
### Each Cell-type Sorted by Abundance (log2FC > 2)

```{r, fig.width = 15}
markers <- read.delim(file.path(assets_dir, "gene-sets", "markers.tsv"))
markers <- markers[markers$p_val_adj < 0.05 & markers$avg_log2FC > 2, ]
markers <- markers %>% distinct(gene, .keep_all = TRUE)
markers_orig <- markers
...
### Most Abundant Markers (log2FC > 2)
...
### Each Cell-type Sorted by Abundance (log2FC > 5)

```{r, fig.width = 15}
markers <- read.delim(file.path(assets_dir, "gene-sets", "markers.tsv"))
markers <- markers[markers$p_val_adj < 0.05 & markers$avg_log2FC > 5, ]
markers <- markers %>% distinct(gene, .keep_all = TRUE)
markers_orig <- markers
...
### Most Abundant Markers (log2FC > 5)
...
MISEV <- read.delim(file.path(assets_dir, "fractions", "MISEV-master-list.tsv"))
MISEV <- MISEV[ , -31]
...
colnames(comb_data) <- paste0("Fraction ", 1:8)
...
for (i in seq_along(colnames(MISEV))) {
  if (length(grep("[*]", MISEV[1, i])) == 1) {
    ...
    colnames(data_list[[i]]) <- paste0("Fraction ", 1:8)
...
for (i in seq_along(unique(categories))) {
  ...
  rep_new <- c()
  for (j in seq_along(rep_names)) {
    rep_sub <- rep[which(order(rep_names) < order(rep_names)[j])]
    rep_new <- c(rep_new, sum(rep_sub) + seq(rep[j]))
  }

  data_list[[i]] <- data_sub[rep_new, ]
  data_list[[i]] <- data_list[[i]][order(rowMedians(data_list[[i]]), decreasing = TRUE), ]
  categories_sub_new <- c(categories_sub_new, rep(names(rep), rep))
}
