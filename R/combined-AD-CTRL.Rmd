...
data <- data[order(rowMeans(assay(data), TRUE)), ]
...
discard <- which(
  rowSums(missing_data[ , 1:ceiling(ncol(missing_data) / 2)], TRUE) <= 4
)
data <- data[-discard, ]
...
missing_data <- ifelse(is.na(missing_data), 0, 1)
discard <- which(
  rowSums(missing_data[ , (ceiling(ncol(missing_data) / 2) + 1):ncol(missing_data)], TRUE) <= 4
)
data <- data[-discard, ]
...
plot_heatmap_custom(data_results, "centered", TRUE, round((nrow(sig) - 1) / 2), row_font_size = 9)
plot_heatmap(data_results, "centered", TRUE, round((nrow(sig) - 1) / 2), row_font_size = 9)
...
plot_heatmap_custom(complete_results, "centered", TRUE, round((nrow(sig) - 1) / 2), row_font_size = 9)
plot_heatmap(complete_results, "centered", TRUE, round((nrow(sig) - 1) / 2), row_font_size = 9)
