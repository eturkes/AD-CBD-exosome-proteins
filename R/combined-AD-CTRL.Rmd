...
design <- read.csv(file.path(assets_dir, data_name, "metadata.csv"))
design$replicate <- c(seq(9), seq(8))

data <- make_unique(data, "Gene.names", "Protein.IDs.x", ";")
keep <- grep("Intensity.", colnames(data))
...
long_names <- colnames(data_minimal)[seq(nrow(design))]
rm(data_minimal)
...
Control_MNAR <- rownames(data)[1:which(rownames(data) == "PREP")] # MGST1
plot_missval_custom(data)
...
colnames(complete_results) <- short_names
...
Heatmap(
  ...
  column_names_side = "top",
  heatmap_legend_param = list(
    title = "Z-score enrichment", title_position = "leftcenter-rot"
  ),
)
...
Heatmap(
  ...
  column_names_side = "top",
  heatmap_legend_param = list(
    title = "SSGSEA enrichment", title_position = "leftcenter-rot"
  ),
)
