---
title: "Combined AD CTRL No Mito"
author:
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(
    "..", "results", "combined-AD-CTRL-no-mito.html"
  ))})
---
...
We remove all mitochondrial proteins for this version of the analysis.
...
# Only Remove MT Prefixed Proteins

For this version we only remove proteins prefixed with `MT_` or `MT-`.
...
# Remove mitochondrial proteins.
# ------------------------------
remove <- c(grep("MT-", data$Gene.names), grep("MT_", data$Gene.names))
cat(paste("Removing", length(remove), "mitochondrial genes:", sep = " "), data$Gene.names[remove])
data <- data[-remove, ]
# ------------------------------
...
# --------------------------------------------------
discard <- which(is.na(rowMeans(assay(data)[ , 1:9], TRUE)))
data <- data[-discard, ]
discard <- which(is.na(rowMeans(assay(data)[ , 10:17], TRUE)))
data <- data[-discard, ]
# --------------------------------------------------
...
# values.
# --------------------------------------------------------------------------------------------------
plot <- plot_detect_custom(data[ , 1:9])
plot_data <- ggplot_build(plot)
MNAR_cutoff <- plot_data$data[[2]]$xintercept
cat(paste0("For AD condition.\nCutoff at ", MNAR_cutoff))
plot

plot <- plot_detect_custom(data[ , 10:17])
plot_data <- ggplot_build(plot)
MNAR_cutoff <- plot_data$data[[2]]$xintercept
cat(
  paste0(
    "For Control condition.\nCutoff at ", MNAR_cutoff,
    "\nWe use this cutoff for downstream analysis."
  )
)
plot
# --------------------------------------------------------------------------------------------------
...
# protein is not MNAR.
# --------------------------------------------------------------------------------------------
AD_order <- order(rowMeans(assay(data)[ , 1:9], TRUE))
data <- data[AD_order, ]
AD_MNAR <- names(which(rowMeans(assay(data)[ , 1:9], TRUE) < MNAR_cutoff))
plot_missval_custom(data, AD_MNAR)

Control_order <- order(rowMeans(assay(data)[ , 10:17], TRUE))
data <- data[Control_order, ]
Control_MNAR <- names(which(rowMeans(assay(data)[ , 10:17], TRUE) < MNAR_cutoff))
plot_missval_custom(data, Control_MNAR)
# --------------------------------------------------------------------------------------------
...
Here we perform VSN normalization, but also include some output from NormalyzerDE web tool [@willforssNormalyzerDEOnlineTool2019] ([http://130.235.214.136/](http://130.235.214.136/)) for evaulating the performance of various normalization methods (note that these figures were only generated without outlier removal and without mitochondrial genes removed).
...
EnhancedVolcano(
  ...
  col = c(
    "grey80", brewer.pal(10, "RdYlBu")[4], brewer.pal(10, "RdYlBu")[4], brewer.pal(10, "RdYlBu")[1]
  ),
  legendLabels = c(
    "No thresholds passed", expression("|"~log[2]~"fold change |" > 1),
    expression("Adjusted p" < 0.05), "Both thresholds passed"
  ),
...
# Remove Mito from Johnson 2020 Modules

For this version we remove all proteins within the mitochondria module from [@johnsonLargescaleProteomicAnalysis2020].
We still removed `MT` prefixed proteins as well.
...
# Remove Johnson 2020 mito module proteins.
# -----------------------------------------
gene_sets <- getGmt(file.path(assets_dir, "gene-sets", "proteomics-modules.gmt"))
remove <- which(data$Gene.names %in% gene_sets[["M3-mitochondrial"]]@geneIds)
cat(paste("Removing", length(remove), "mito module genes:", sep = " "), data$Gene.names[remove])
data <- data[-remove, ]
# -----------------------------------------
...
# --------------------------------------------------------
gene_sets <- getGmt(file.path(assets_dir, "gene-sets", "proteomics-modules.gmt"))
gene_sets[-(which(names(gene_sets) == "M3-mitochondrial"))]
gene_sets
# --------------------------------------------------------
...
# Remove Mito from Custom List

For this version we use a custom list of mitochondrial proteins.
We still removed `MT` prefixed proteins as well.
...
# -----------------------------------------
mito <- read.xlsx(
  file.path(assets_dir, "gene-sets", "Mitochondrial proteins to exclude.xlsx"), colNames = FALSE
)
remove <- which(data$Gene.names %in% mito$X1)
cat(paste("Removing", length(remove), "mitochondrial genes:", sep = " "), data$Gene.names[remove])
data <- data[-remove, ]
# -----------------------------------------
