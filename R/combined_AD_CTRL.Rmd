...
packages <- c(
  ...
  "DT", "ComplexHeatmap", "GSEABase", "GSVA", "RColorBrewer", "limma"
)
...
# No Outlier Removal & Publication Ready Figs

In this section we look at all of the data without any removal of possible outlier samples.
Some figures are stripped of additional information to make them more suitable for publication.
...
sex_color <- hue_pal()(length(unique(design$sex)))
month_color <- hue_pal()(length(unique(design$month)))
top_anno <- HeatmapAnnotation(
  ...
  col = list(
    biological_replicate = c(
      "AD_5333" = bio_rep_color[1], "AD_5365" = bio_rep_color[2],
      "AD_5398" = bio_rep_color[3], "AD_5475" = bio_rep_color[4],
      "AD_5521" = bio_rep_color[5], "CTRL_159" = bio_rep_color[6],
      "CTRL_4070" = bio_rep_color[7], "CTRL_5382" = bio_rep_color[8],
      "CTRL_328" = bio_rep_color[9]
    ),
    sex = c("M" = sex_color[1], "F" = sex_color[2]),
    month = c("March" = month_color[1], "Jan" = month_color[2])
  ),
...
Here we perform VSN normalization, but also include some output from NormalyzerDE [@willforssNormalyzerDEOnlineTool2019] (web tool at [http://130.235.214.136/](http://130.235.214.136/)) for evaulating the performance of various normalization methods.
...
We use the `variancePartition` package to quantify and interpret drivers of variation.

**All Covariates**

```{r}
formula <- ~ age + (1|sample) + (1|month) + (1|sex) + (1|region) + (1|condition)
...
We filter using a 0.05 p-value cutoff after BH multiple testing correction.
We also test DE while correcting for several combinations of covariates.

### No Covariates Regressed
...
```{r}
heatmap_data <- t(scale(t(assay(data)[rownames(data) %in% sig$name, ])))
color2 <- colorRamp2(
...
data_results <- add_rejections(
  test_diff(data, "control", "Control", design_formula = ~ 0 + condition + month),
  lfc = 0
)
results <- get_results(data_results)
sig <- results[results$significant, ]
...
# -----------------------
data_results <- add_rejections(
  test_diff(data, "control", "Control", design_formula = ~ 0 + condition + sex + month),
  lfc = 0
)
results <- get_results(data_results)
sig <- results[results$significant, ]
...
### Sex Regressed Out (Used Downstream)

```{r}
# Perform the DE testing.
# -----------------------
data_results <- add_rejections(
  test_diff(data, "control", "Control", design_formula = ~ 0 + condition + sex),
  lfc = 0
)
results <- get_results(data_results)
sig <- results[results$significant, ]
...
complete_results <- add_rejections(
  test_diff(
    complete_cases, "control", "Control", design_formula = formula(~ 0 + condition + sex)
  ),
  lfc = 0
)
...
`limma` is applied following these methods to test if differences are significant, note that while this approach is described in the `GSVA` vignette, I am not sure if it is applicable to all methods in the package.
...
# -----------------------------------
limma_design <- model.matrix(~ 0 + design$condition + design$sex, as.data.frame(gsva))
...
**Plotting individual genes for each gene set.**
