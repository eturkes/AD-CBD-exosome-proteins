...
heatmap(
  gsva, ColSideColors = sample_color_map, margins = c(6, 10), col = color,
  labCol = colnames, labRow = rownames(gsva), Rowv = as.dendrogram(gene_set_clustering), Colv = NA
)
...
gene_set_clustering <- hclust(as.dist(1 - cor(t(gsva), method = "pearson")), "complete")
heatmap(
  gsva, ColSideColors = sample_color_map, margins = c(6, 25), col = color,
  labCol = colnames, labRow = rownames(gsva), Rowv = as.dendrogram(gene_set_clustering), Colv = NA
)
legend("topleft", names(color_legend), fill = color_legend, inset = 0.01, bg = "white")
```
