...
**NOTE: The results below are NOT significant.**
...
fit <- eBayes(lmFit(gsva, model.matrix(~ groups)))
DE_gene_sets <- topTable(fit, paste0("groups", conditions[2]), Inf, p.value = 0.11)
datatable_download_exp(DE_gene_sets)
...
fit <- eBayes(lmFit(gsva, model.matrix(~ groups)))
DE_gene_sets <- topTable(fit, paste0("groups", conditions[2]), Inf, p.value = 0.133)
datatable_download_exp(DE_gene_sets)
