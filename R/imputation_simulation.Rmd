...
pattern <- matrix(nrow = 256, ncol = ncol(data))
for (fraction in unique(data$condition)) {
  idx <- which(data$condition == fraction)
  set.seed(idx[1])
  pattern[ , idx] <- as.matrix(
    expand.grid(replicate(length(idx), 0:1, simplify = FALSE))[sample(256), ]
  )
}
freq <- sum(rowSums(pattern)) / rowSums(pattern) / sum(sum(rowSums(pattern)) / rowSums(pattern))
pattern <- pattern[order(freq, decreasing = TRUE), ][seq(ncol(data)), ]
freq <- sum(rowSums(pattern)) / rowSums(pattern) / sum(sum(rowSums(pattern)) / rowSums(pattern))

set.seed(1)
sim_data <- as.matrix(ampute(sim_data, 0.15, pattern, freq, mech = "MCAR")$amp)
