...
```{r}
sim_list <- vector("list", length(unique(data$condition)))
names(sim_list) <- unique(data$condition)
for (fraction in unique(data$condition)) {
  data_sub <- assay(data)[ , which(data$condition == fraction)]

  mu <- rowMeans(data_sub, TRUE)
  mu[is.na(mu)] <- min(mu, na.rm = TRUE)
  std <- apply(data_sub, 1, sd, na.rm = TRUE)
  std[is.na(std)] <- std[order(mu)][which(!is.na(std[order(mu)]))[1]]

  set.seed(1)
  sim_list[[fraction]] <- suppressWarnings(
    t(as.data.frame(mapply(function(x, y) rnorm(ncol(data_sub), x, y), mu, std)))
  )
}
sim_data <- do.call("cbind", sim_list)
hist(sim_data, n = 100)
