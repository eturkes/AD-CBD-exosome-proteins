...
amp_MAR <- ampute(
  ...
  0.7,
  pattern,
  sum(rowSums(pattern)) / rowSums(pattern) / sum(sum(rowSums(pattern)) / rowSums(pattern)),
  mech = "MAR"
)
set.seed(1)
amp_MNAR <- ampute(
  ...
  0.7,
  pattern,
  rowSums(pattern) / sum(rowSums(pattern)),
  mech = "MNAR"
)

set.seed(1)
idx <- sample(c(1, 2), nrow(data), TRUE, c(0.25, 0.75))
...
assay(data) <- amp_data
rm(sim_data, amp_data, amp_MAR, amp_MNAR)
data
