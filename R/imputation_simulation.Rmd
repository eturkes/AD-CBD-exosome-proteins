...
pattern <- matrix(nrow = 93, ncol = ncol(data))
for (fraction in unique(data$condition)) {
  ...
  pattern[ , idx] <- as.matrix(
    expand.grid(replicate(length(idx), 0:1, simplify = FALSE))[
      rowSums(expand.grid(replicate(length(idx), 0:1, simplify = FALSE))) >= 5,
    ][sample(93), ]
  )
}
...
set.seed(1)
sim_data <- as.matrix(ampute(sim_data, 0.15, pattern, freq, "MCAR")$amp)

below_LOD <- which(sim_data < 8.5)
set.seed(1)
below_LOD <- sample(below_LOD, 1e6, TRUE, (1 / sim_data[below_LOD]) ^ 6)
