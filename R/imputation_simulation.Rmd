...
pattern <- matrix(nrow = 163, ncol = ncol(data))
for (fraction in unique(data$condition)) {
  ...
  pattern[ , idx] <- as.matrix(
    expand.grid(replicate(length(idx), 0:1, simplify = FALSE))[
      rowSums(expand.grid(replicate(length(idx), 0:1, simplify = FALSE))) >= 4,
    ][sample(163), ]
  )
}
if (any(rowSums(pattern) == 0)) {
  pattern <- pattern[-(rowSums(pattern) == 0), ]
}
...
below_LOD <- sample(below_LOD, 1e7, TRUE, (1 / sim_data[below_LOD]) ^ 9)
