...
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(
    "..", "results", "imputation_simulation.html"
  ))})
---
...
packages <- c(
  ...
  "ComplexHeatmap", "volcano3D", "RColorBrewer", "limma", "tibble", "khroma", "mice"
)
...
cache_dir <- file.path("..", "tmp", "cache", "simulation")
...
LFQ_columns <- 6:69
data <- data[ , c(1:5, (order(substring(colnames(data)[LFQ_columns], 2, 2)) + 5), 70:ncol(data))]
...
# min_vector <- vector("numeric", ncol(sim_data))
# for (i in seq_along(min_vector)) {
#   min_vector[i] <- min(sim_data[ , i])
# }

min_vector <- vector("numeric", length(unique(data$condition)))
for (i in seq_along(min_vector)) {
...
for (fraction in unique(data$condition)) {
  ...
  set.seed(idx[1])
  pattern[ , idx] <- as.matrix(
    expand.grid(replicate(length(idx), 0:1, simplify = FALSE))[
      rowSums(expand.grid(replicate(length(idx), 0:1, simplify = FALSE))) >= 1,
    ][sample(255), ]
  )
}
if (any(rowSums(pattern) == ncol(data))) {
  pattern <- pattern[-(rowSums(pattern) == ncol(data)), ]
}
freq <- rowSums(pattern) / sum(rowSums(pattern))
# freq <- sum(rowSums(pattern)) / rowSums(pattern) / sum(sum(rowSums(pattern)) / rowSums(pattern))

set.seed(1)
sim_data <- as.matrix(ampute(sim_data, 0.05, pattern, freq, "MCAR")$amp)
...
# below_LOD <- which(sim_data < 10.5)
set.seed(1)
# below_LOD <- sample(below_LOD, 1e6, TRUE, (1 / sim_data[below_LOD]) ^ 8)
# below_LOD <- sample(below_LOD, 5e8, TRUE, (1 / sim_data[below_LOD]) ^ 9)
below_LOD <- sample(below_LOD, 5e6, TRUE, (1 / sim_data[below_LOD]) ^ 7.5)
...
#       assay(data)[replace[i], col[i]] <- min_vector[col[i]]
#     }
#   }
#   saveRDS(data, rds)
# }
