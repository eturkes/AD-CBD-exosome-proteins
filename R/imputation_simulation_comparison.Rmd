...
```{r}
venn_list <- list(
  "imputed" = results_list[[1]]$X[results_list[[1]]$F.p.value < 0.05],
  "unimputed" = results_list[[2]]$X[results_list[[2]]$F.p.value < 0.05],
  "complete" = results_list[[3]]$X[results_list[[3]]$F.p.value < 0.05]
)
...
for (name in names(venn_list)) {
  ...
  set.seed(1)
  mat <- assay(
    data_list[[name]][rownames(data_list[[name]]) %in% sample(venn_list[[name]], 40), ]
  )
...
venn_list_true <- list(
  "imputed" = results_list[[1]]$X[results_list[[1]]$F.p.value < 0.05][
    as.numeric(gsub("\\D+","", results_list[[1]]$X[results_list[[1]]$F.p.value < 0.05])) <= 3000
  ],
  "unimputed" = results_list[[2]]$X[results_list[[2]]$F.p.value < 0.05][
    as.numeric(gsub("\\D+","", results_list[[2]]$X[results_list[[2]]$F.p.value < 0.05])) <= 3000
  ],
  "complete" = results_list[[3]]$X[results_list[[3]]$F.p.value < 0.05][
    as.numeric(gsub("\\D+","", results_list[[3]]$X[results_list[[3]]$F.p.value < 0.05])) <= 3000
  ]
)
...
```{r}
df <- data.frame(
  Imputed = round(length(venn_list_true$imputed) / 3000 * 100, 2),
  Unimputed = round(length(venn_list_true$unimputed) / 3000 * 100, 2),
  Complete = round(length(venn_list_true$complete) / 3000 * 100, 2)
)
