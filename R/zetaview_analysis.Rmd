...
We then output this object for download along with variations that are aggregated by the mean of values by donor and fraction.
...
```{r}
molten_data <- melt(donor_fraction_data[ , -1], id.vars = NULL)
colnames(molten_data) <- c("sample", "frequency")
molten_data$size <- rep(donor_fraction_data$size, ncol(donor_fraction_data) - 1)
```
...
```{r}
molten_data <- melt(donor_data[ , -1], id.vars = NULL)
colnames(molten_data) <- c("sample", "frequency")
molten_data$size <- rep(donor_data$size, ncol(donor_data) - 1)
```
...
```{r}
molten_data <- melt(fraction_data[ , -1], id.vars = NULL)
colnames(molten_data) <- c("sample", "frequency")
molten_data$size <- rep(fraction_data$size, ncol(fraction_data) - 1)
```
