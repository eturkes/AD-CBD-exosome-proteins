...
```{r}
data_sub <- colSums(fraction_data[ , -1])
data_sub <- data.frame(Fraction = names(data_sub), Concentration = data_sub)

ggplot(data_sub, aes(Fraction, Concentration, fill = Fraction)) +
  geom_bar(stat = "identity") +
  theme_light() +
  theme(legend.position = "none") +
  scale_y_continuous(
    breaks = seq(0, max(data_sub$Concentration), 1e+12),
    limits = c(0, max(data_sub$Concentration))
  ) +
...
```{r}
data_sub <- colSums(fraction_data_scaled[ , -1])
data_sub <- data.frame(Fraction = names(data_sub), Concentration = data_sub)
